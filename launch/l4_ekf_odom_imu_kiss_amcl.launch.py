...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=['ros2', 'bag', 'play', bag_path, '--clock', '--delay', '3.0', '--remap', '/tf_static:=/tf_static_old', '/tf:=/tf_old'],
            output='screen'
        ),
        ...
        # Retranslator to create a path from L3 EKF output
        Node(
            package='b2_thesis_fusion',
            executable='path_tracker',
            name='tracker_l3',
            parameters=[{'odom_topic': '/odometry/filtered_l4', 'path_topic': '/trajectory/l4', 'frame_id': 'odom', 'use_sim_time': use_sim_time}],
        ),


        # Map Server (Publishing saved 2d map to the topic /map)
        Node(
        ...
        Node(
            ...
            parameters=[{
                ...
                'global_frame_id': 'map',
                'scan_topic': '/scan',
                'set_initial_pose': True, # Set programm initial pose
                'initial_pose': [0.0, 0.0, 0.0], # Initial pose (x, y, yaw)
            }]
        ),

        # Lifecycle Manager (activate map server to publish the map)
        Node(
